...
def load_training_dataset():
    # Check if the 'Date' column exists in the CSV file
    needed_columns = {'Date', 'Category', 'Amount', 'Description'}
    training_dataset = pd.read_csv('expenses_ced@gmail.com.csv')

    missing_columns = needed_columns - set(training_dataset.columns)
    if missing_columns:
        raise KeyError(f"Missing columns in the training dataset: {missing_columns}")

    return training_dataset

# Prepare dataset for training
def prepare_dataset(expenses_df):
    ...
    aggregated_expenses = expenses_df.groupby('YearMonth', as_index=False)['Amount'].sum()
    aggregated_expenses['MonthIndex'] = range(len(aggregated_expenses))
...
def forecast_expenses(model, data, future_months=6):
    last_month_index = data['MonthIndex'].max()
    ...
    last_date = pd.to_datetime(data['YearMonth'].max())
...
def plot_budget_trend(budget_df, forecast_df):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(budget_df['YearMonth'], budget_df['Budget'], marker='o', linestyle='-', color='blue', label='Budget')
    ax.plot(budget_df['YearMonth'], budget_df['Amount'], marker='o', linestyle='-', color='orange', label='Actual Expense')
    ax.plot(forecast_df['YearMonth'], forecast_df['Expense'], marker='o', linestyle='--', color='red', label='Predicted Expense')

    ax.set_title('Expense Trend & Forecast')
...
def main_menu(email):
    try:
        ...
        prepared_data = prepare_dataset(training_dataset)

        # Train the model
        model = train_linear_regression_model(prepared_data)

        # Forecast future expenses
        forecast_df = forecast_expenses(model, prepared_data)
        ...
        def show_budget_trend():
            plot_budget_trend(prepared_data, forecast_df)
