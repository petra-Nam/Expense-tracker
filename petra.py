...
def plot_budget_trend(budget_df, forecast_df, canvas):
    # Combine historical and forecast data
    combined_df = pd.concat([budget_df, forecast_df])

    # Plot the data
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(combined_df['YearMonth'], combined_df['Budget'], marker='o', linestyle='-', color='skyblue', label='Expenses')
    ax.axvline(x=budget_df['YearMonth'].max(), color='r', linestyle='--', label='Current Date')
    ax.fill_between(combined_df['YearMonth'], 0, combined_df['Budget'], color='lightgreen', alpha=0.5)

    ax.set_title('Budget Trend Over Time')
    ax.set_xlabel('Month')
...
def forecast_expenses(budget_df, df):
    df['YearMonth'] = pd.to_datetime(df['Date']).dt.to_period('M')
    monthly_expenses = df.groupby('YearMonth')['Amount'].sum().reset_index()
    monthly_expenses['YearMonth'] = monthly_expenses['YearMonth'].astype(str)
    monthly_expenses['YearMonth'] = pd.to_datetime(monthly_expenses['YearMonth'])
    monthly_expenses['Month'] = monthly_expenses['YearMonth'].dt.month
    monthly_expenses['Year'] = monthly_expenses['YearMonth'].dt.year
    monthly_expenses['Month_Year'] = monthly_expenses['Year'] * 12 + monthly_expenses['Month']
    ...
    future_months = [monthly_expenses['Month_Year'].max() + i for i in range(1, 7)]
    future_expenses = model.predict(pd.DataFrame({'Month_Year': future_months}))

    # Create a DataFrame for future predictions
    future_df = pd.DataFrame({
        'YearMonth': pd.date_range(start=monthly_expenses['YearMonth'].max() + pd.offsets.MonthBegin(1), periods=6, freq='MS'),
        'Budget': future_expenses
    })
...
def select_month_year():
    year = simpledialog.askinteger("Select Year", "Enter the year (e.g., 2023):")
    if year is None:
        return None, None
    month = simpledialog.askinteger("Select Month", "Enter the month (1-12):")
...
def main_menu(email, canvas):
    ...
    def show_budget_trend():
        ...
        plot_budget_trend(budget_df, forecast_df, canvas)
    ...
    buttons = [
        ...
        ("Predict Future Expenses", show_budget_trend),
