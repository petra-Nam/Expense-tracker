...
def prepare_budget_data_for_trend(data):
    ...
    return monthly_expenses['Amount']
...
def train_and_predict_model(data, future_months=6):
    ...
    forecast_df = pd.DataFrame({'Date': [monthly_expenses.index.max() + relativedelta(months=i) for i in range(1, future_months + 1)], 'Predicted_Expense': forecast})
...
def plot_budget_trend(monthly_expenses, forecast_df, budget):
    ...
    plt.plot(monthly_expenses.index, monthly_expenses, label='Historical Data', marker='o')
