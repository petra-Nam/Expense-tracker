...
def plot_budget_trend(df, canvas):
    ...
    combined_df = pd.concat([monthly_expenses[['YearMonth', 'Amount']], future_df])

    # Plot the data
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(combined_df['YearMonth'], combined_df['Amount'], marker='o', linestyle='-', color='skyblue', label='Expenses')
    ax.axvline(x=monthly_expenses['YearMonth'].max(), color='r', linestyle='--', label='Current Date')
    ax.fill_between(combined_df['YearMonth'], 0, combined_df['Amount'], color='lightgreen', alpha=0.5)
